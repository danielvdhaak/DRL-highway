...
public class Driver : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] [Range(50,200)] private int measureDistance = 100;
    [SerializeField] private float k = 50f;
    [SerializeField] private float kt = 1.0f;
    [SerializeField] private float kv = 60f;
    [SerializeField] private float kd = 30f;
    ...
    private void ACC()
    {
        ...
        {
            if (Physics.Raycast(origin, direction, out hit, measureDistance) && hit.transform.tag == "Car")
            {
                ...
                Driver carInfrontDriver = hit.transform.GetComponent<Driver>();
                float carInfrontThrottle = carInfrontDriver.throttle;
                

                // Determine minimum and throttle
                freeThrottle = k * (desiredVelocity - velocity);
                referenceThrottle = kt * carInfrontThrottle + kv * ((carInfrontVelocity - velocity)/3.6f) + kd * (hit.distance - gap);

                throttle = Mathf.Min(freeThrottle, referenceThrottle);
            }
            else
            {
                throttle = k * (desiredVelocity - velocity);
            }
        }
        else
        {
        ...
        // Apply throttle
        float motorTorque = Mathf.Clamp(throttle, 0f, 1000f);
        float brakeTorque = Mathf.Clamp(throttle, -1000f, 0f);
